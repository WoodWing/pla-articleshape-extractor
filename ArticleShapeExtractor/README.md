# Introduction
Article Shape Extractor tool for the Print Layout Automation (PLA) project. 

The tool provides the following functionality:
* Administration:
  * Tagging for article shapes.
  * Export the content of a single layout file (opened in Adobe InDesign) to article shapes.
  * Batch wise regenerate of the content of multiple layout (based on status) files to article shapes.
* Workflow:
...
> ***Optional*** - These configuration steps are only needed if you want to use the administration functionality.
...
# Usage
There are three ways of using this tool, depending on the process stage:
...
| 3 | Fitting of article content | For a placed article, make the content fit with AI.
...
## Phase 3 - Fitting of article content
When a new layout is generated by the Layout Automation tool, it has placed articles. The content of such article may not be perfectly fitted yet. To automatically resolve its underset/overset, AI can be invoked (again):
1. Open the layout and locate the placed article.
2. Right mouse click any of its frames to open the context menu.
3. Select the `Fitting` > `Fit Article with AI` menu item.
